...
try:
    rq = requests.get('https://pastebin.com/raw/2ceqHj0i.txt').text
...
def started():
    ...
    print('%s [%s•%s] %s[%sOK%s] IDS SAVED IN >> %AYAZ-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sCP%s] IDS SAVED IN >> %AYAZ-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
    psb('%s [%s•%s] %sON FLIGHT MODE FOR [5 SEC] EVERY 5 MINUTES\n'%(H,P,H,B))


def folder():
    try:os.mkdir("AYAZ-CP")
    except:pass
    try:os.mkdir("AYAZ-OK")
    except:pass
...
def genkey():
	try:
		...
		subprocess.check_output(["am", "start", "https://wa.me/+923312734753"])
		exit()
	except(IOError):
		rb = rx
